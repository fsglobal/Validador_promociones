...
def resolver_regla(
    ...
    # Modalidad
    if modalidad in (MODALIDAD_MASIVA, MODALIDAD_EVENTOS):
        regla = _aplicar_masiva(regla, area_funcional)
        if modalidad == MODALIDAD_EVENTOS:
            regla["basico"]["modalidad"] = MODALIDAD_EVENTOS
            regla["tiempo"]["usa_dias_especificos"] = True
            regla["tiempo"]["observaciones"].append(
                "En EVENTOS se deben revisar fechas y días específicos."
            )
            regla["condiciones"]["locales_regla"] = "Locales asignados"
            regla["condiciones"]["observaciones"].append(
                "En EVENTOS usar locales asignados en lugar de EXC_LOCALES."
            )
            regla["alertas"].append(
                "EVENTOS usa locales asignados y requiere revisión de fechas/días específicos."
            )
    else:
    ...
    if tipo_descuento == TIPO_PACK_ESPECIAL_BYCP:
        regla["alertas"].append(
            "PACK ESPECIAL BYCP: condición 3, porcentaje 100 a 1 unidad, strategy menor y competencia por producto."
        )
